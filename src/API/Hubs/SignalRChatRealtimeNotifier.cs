    ...
    ILogger<SignalRChatRealtimeNotifier> logger) : IChatRealtimeNotifier
{
    ...
    public Task GroupUpdatedAsync(int chatId, string title, string? description, int? photoId, int memberCount, IReadOnlyCollection<int> ids, CancellationToken cancellationToken = default) => PublishAsync("GroupUpdated", new { chatId, title, description, photoId, memberCount }, ids, cancellationToken);
