...
public sealed class MessageAttachmentRepository(ChatDb db) : IMessageAttachmentRepository
{
    ...
    public Task<Message?> GetMessageAsync(int messageId, CancellationToken cancellationToken = default) => db.Messages.AsNoTracking().Include(message => message.Sender).ThenInclude(sender => sender.OrganizationMembership!).ThenInclude(membership => membership.Organization).Include(message => message.Attachments).ThenInclude(attachment => attachment.File).FirstOrDefaultAsync(message => message.Id == messageId, cancellationToken);
